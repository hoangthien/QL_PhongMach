    ...
    class ToaThuoc
    {
        ...
        public static List<ChiTietToaThuoc> LayChiTietDonThuoc(int MaPK)
        {
            ...
            if (dl.MoKetNoi())
            {
                ...
                List<ChiTietToaThuoc> ct = new List<ChiTietToaThuoc>();
                int stt = 0;
                while (dr.Read())
                {
                    stt++;
                    ct.Add(new ChiTietToaThuoc((int)dr["MaThuoc"], stt, dr["TenThuoc"].ToString(), dr["DonVi"].ToString(), (int)dr["SoLuong"], dr["CachDung"].ToString()));
                }
                dl.DongKetNoi();
                return ct;
            }
