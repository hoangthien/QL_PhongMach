...
namespace QLPhongMach
{
    class PhieuKham
    {
        ...
        public static List<ChiTietPhieuKham> DSKhamBenh(string ngayKham)
        {
            DuLieu dl = new DuLieu();
            List<ChiTietPhieuKham> pk = new List<ChiTietPhieuKham>();
            if (dl.MoKetNoi())
            {
                string sqlString = "select TenBN as 'Họ tên', NgayKham as 'Ngày Khám', LoaiBenh as 'Loại bệnh', TrieuChung as 'Triệu chứng' from BenhNhan bn join PhieuKham pk on bn.MaBN = pk.MaBN where NgayKham = '" + ngayKham + "'";
